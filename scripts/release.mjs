...
if(old?.day===day&&marker?.day!==day){console.log('Riprendo lo stesso snapshot non ancora confermato:',old.generation);stage();process.exit(0);}
if(marker?.day===day){console.log('Giornata già pubblicata: nessuna nuova esecuzione.');stage();process.exit(0);}
...
const settled=ledger.map(p=>settle(p,results.get(p.eventId)));
for(const sport of sports){
  ...
  for(const e of events){
    ...
    const market=noVig(e.bookmakers,[e.home_team,'Draw',e.away_team],now);
    ...
    for(let i=0;i<3;i++){
      const outcome=['1','X','2'][i];const analysis=pred?assess(pred.p[i],market[i],context):null;
...
const bundle={schema:1,generation:randomUUID(),generatedAt:now,day,zone:POLICY.zone,version:VERSION,policy:POLICY,
  ...
  limits:['Nessun esito è certo. Confidence misura la qualità della stima, non la probabilità di vincita.',
    ...
    'Modello esclusivamente xG: nessun dato verificato su formazioni o infortuni. La revisione retrospettiva degli xG limita il backtest.',
...
function stage(){mkdirSync('dist',{recursive:true});for(const name of ['index.html','app.js','style.css'])copyFileSync(name,`dist/${name}`);copyFileSync('data/release.json','dist/release.json');writeFileSync('dist/.nojekyll','');}
